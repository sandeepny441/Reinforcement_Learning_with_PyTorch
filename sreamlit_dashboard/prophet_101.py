...
data = {
    'date': pd.date_range(start='2023-01-01', periods=100, freq='D'),  # 100 days of data
    'value': [x + (x * 0.1) + np.sin(x / 5) * 5 for x in range(100)]  # Adding a trend + sine wave for variability
}
...
plt.plot(df['ds'], df['y'], label='Historical Data', color='blue', linewidth=2)  # Historical data
plt.plot(forecast['ds'], forecast['yhat'], label='Forecast', color='green', linestyle='--')  # Forecast line
plt.fill_between(
    forecast['ds'], forecast['yhat_lower'], forecast['yhat_upper'], color='lightgreen', alpha=0.4, label='Confidence Interval'
)  # Confidence interval

# Add titles and labels
plt.title("Prophet Forecast with Custom Colors and Variability", fontsize=16)
